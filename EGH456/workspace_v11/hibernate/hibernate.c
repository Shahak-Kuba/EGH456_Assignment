...
volatile uint32_t buttonPressed = false;
volatile uint32_t secondsPassed = 0;
volatile uint32_t time_since_start = 0;
...
//
//*****************************************************************************
char g_pcWakeBuf[40], g_pcHibBuf[40], g_pcDateTimeBuf[40], g_pcButtonPress[40];
...
SysTickIntHandler(void)
{
    ...
    switch(ui32Buttons & ALL_BUTTONS)
    {
        ...
        case USR_SW1:
        {
            ...
            g_bHibernate = false;
            buttonPressed = true;
            //button_counter++; // increasing overall button press count
...
{
    if (secondsPassed == 10) //10 seconds have passed without a press
    {
        secondsPassed = 0;
        g_bHibernate = true;
        UARTprintf("\n 10 seconds passed \n");
    }
    else if (buttonPressed == false && secondsPassed < 10) // updating seconds since button pressed counter
    {
        secondsPassed++;
        //UARTprintf("\n added second to timer counter \n");
    }
    else if (buttonPressed == true)
    {
        ...
        buttonPressed = false;
        g_bHibernate = false;
        UARTprintf("\n button pressed restart counter \n");
    }
    //
...
main(void)
{
    ...
    ROM_TimerLoadSet(TIMER0_BASE, TIMER_A, ui32SysClock * 1); // 1Hz
    ...
    /*
    // Store the number of times button has been pressed
    usnprintf(g_pcButtonPress, sizeof(g_pcButtonPress), "Button count = %u",
              button_counter);*/
    ...
    while(1)
    {
        ...
        if(bUpdate == true)
        {
            ...
            UARTprintf("The time passed is: %d seconds\n", time_since_start);
            UARTprintf("\033[K");
            UARTprintf("%s\n", g_pcHibBuf);
            UARTprintf("%s\n", g_pcButtonPress);
            UARTprintf("\033[K");
